...
def update_data(): # Funcția pentru actualizarea datelor
    ...
    note = Note.query.filter_by(date=data['date']).first() # Căutăm înregistrarea cu data specificată în baza de date
...
def home(): # Funcția pentru pagina principală
    ...
    if request.method == 'POST': # Verificăm dacă s-a trimis o cerere de tip POST
        ...
        existing_entry = Note.query.filter_by(date=date, user_id=current_user.id).first() # Verificăm dacă există deja o înregistrare pentru această dată
        if existing_entry: # Verificăm dacă există deja o înregistrare pentru această dată
            flash('The entry for this date already exists!', category='error') # Afisăm un mesaj de eroare dacă există deja o înregistrare pentru această dată
            return redirect(url_for('views.home')) # Redirecționăm utilizatorul la pagina principală
        elif len(hours) < 1: # Verificăm dacă numărul de ore este mai mic decât 1
            flash('Note is too short!', category='error') # Afisăm un mesaj de eroare dacă numărul de ore este mai mic decât 1
        else: # Dacă nu există deja o înregistrare pentru această dată și numărul de ore este mai mare sau egal cu 1
